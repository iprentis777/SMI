...
| `Reports/REPORT_CSD_response_derivation.md` | **how the CSD response is derived**, at methods-section depth: the two conventions, `r_l = K_l sqrt((2l+1)4pi)`, the Watson dispersion correction, why the healthy kernel is used for both tissues, why orientation is invariant to the response but amplitude is not, and the assumptions |
