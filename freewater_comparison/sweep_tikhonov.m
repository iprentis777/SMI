...
LAMBDAS  = [0 0.05 0.1 0.2 0.3 0.5 0.8];
LMAX_FIT = 6;
CS       = 1;
D_FW     = 3;
...
for il = 1:numel(LAMBDAS)
    ...
    flm(~isfinite(flm)) = 0;
    IO.save(sprintf('smi_amp_lam%d_%s', il, tag), Y*flm');
    IO.save(sprintf('smi_pl_lam%d_%s', il, tag), ...
            reshape(out.pl, [NVOX size(out.pl,4)]));
    fprintf('lambda_tikhonov %.2f  ->  %.1f s\n', lam, toc(t0));
end
IO.save(['tik_lambdas_' tag], LAMBDAS);
end
