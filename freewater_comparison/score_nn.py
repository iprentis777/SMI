...
def run(tag, snr):
    ...
    cid = cs.cond_id()
    axes = cs.gt_axes()
    lam = binio.load(f'nn_lambdas_{tag}').ravel()
    ...
    print(f'\n{"="*104}\n  NON-NEGATIVITY SWEEP, SNR {snr}   '
          f'(lambda_tikhonov = 0.3 throughout; lambda 0 = constraint OFF, Lmax 6)\n{"="*104}')
    ...
    for il, L in enumerate(lam, start=1):
        M = binio.load(f'smi_amp_nn{il}_{tag}')
...
if __name__ == '__main__':
    for t, s in [('clean', 'noise free'), ('snr30', '30'), ('snr15', '15')]:
        if binio.exists(f'nn_lambdas_{t}'):
            run(t, s)
